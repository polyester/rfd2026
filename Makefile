...
PROJECT_NAME=rfd2026
STACK_NAME=rozefilmdagen-nl
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://rfd2026.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t polyester/rfd2026-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t polyester/rfd2026-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name rfd2026-frontend-acceptance --link rfd2026-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d polyester/rfd2026-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name rfd2026-backend-acceptance -d polyester/rfd2026-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop rfd2026-frontend-acceptance
	@docker stop rfd2026-backend-acceptance
